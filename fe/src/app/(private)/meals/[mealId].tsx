...
import { calcMicroPercentage } from "../../../utils/calcMacroPercentage";
...
export default function MealDetails() {
  ...
  const { data, isPending } = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      if (Date.now() - startTimeRef.current >= 30_000) { // Aumentar para 30 segundos
        setPollingEnabled(false);
        setIsErrorModalOpen(true);
        return false;
      }
  ...
  if (isPending || data?.meal.status === "processing" || data?.meal.status === "uploading" ) {
    return <LoadingScreen message="Carregando detalhes da refeição..." />;
  }
